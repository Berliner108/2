...
export async function POST(req: Request) {
  try {
    ...
    // 3) Stripe Customer des Käufers
    const customerId = await getOrCreateStripeCustomer(user.id, user.email)

    // 4) Gibt es bereits eine „offene“ Order zu (offerId, buyer_id)? -> Reuse PI
    //    Wir nehmen als „offen“ hier status = 'processing' (passt zu deiner Enum).
...

    {
      const { data: ord, error: ordErr } = await sb
        ...
        .eq('buyer_id', user.id)
        .in('status', ['processing']) // <- falls du mehr „Zwischen“-Status hast, hier ergänzen
        .maybeSingle()
    ...
    // 5) Falls schon ein PI existiert, ClientSecret zurück (so kann der Käufer fortfahren)
    ...
    //    7% Fee nur vormerken; der Abzug erfolgt erst beim Transfer (separate charges & transfers)
    ...
    const { data: ins, error: insErr } = await sb
      ...
      .insert({
        ...
        kind: 'lack',                 // du nutzt „lack“ für Lackanfragen
        request_id: reqRow.id,
        offer_id: offer.id,
        amount_cents: offer.amount_cents,
        currency: offer.currency || 'EUR',
        fee_cents: feeCents,
        status: 'processing',         // muss in deinem Enum existieren
      })
      .select('id')
      .maybeSingle()
    ...
    // 7) PaymentIntent erstellen (Charge auf der Plattform — separate charges & transfers)
