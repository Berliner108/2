...
export async function DELETE(
  ...
  try {
    ...
    // Alle Dateien unter dem Prefix entfernen (best effort, inkl. "Unterordner")
    async function removePrefix(pfx: string) {
      const { data: list, error: lerr } = await admin.storage
        .from(bucket)
        .list(pfx, { limit: 1000, offset: 0 })
      if (lerr) {
        console.warn('[admin delete] list failed', lerr.message)
        return
      }

      const filePaths = (list || []).map(e => `${pfx}${e.name}`)
      if (filePaths.length) {
        const { error: remErr } = await admin.storage.from(bucket).remove(filePaths)
        if (remErr) console.warn('[admin delete] remove failed', remErr.message)
      }

      // Falls "Pseudo-Ordner" mit "/" im Namen auftauchen, rekursiv nachladen
      for (const entry of list || []) {
        if (entry.name?.endsWith('/')) {
          await removePrefix(`${pfx}${entry.name}`)
        }
      }
    }
