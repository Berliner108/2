...
export async function GET(req: Request) {
  try {
    ...
    // ===== Basis-Query: created_at DESC, Overfetch nur wenn sort=promo =====
    let q = supabase
      .from('lack_requests')
    ...
    if (sortParam === 'promo') {
      const factor = 4 // großzügiger overfetch, damit Sortierung greift
      const overTo = page
        ? (page * limit * factor) - 1
    ...
    let items = rows.map((row: any) => {
      ...
      const isSponsored = (promoScoreNum as number) > 0
      ...
      return {
        ...
        // Wichtig: numerisch lassen, kein bitwise-cast!
        promo_score: typeof promoScoreNum === 'number' ? promoScoreNum : Number(promoScoreNum) || 0,
        promoScore:  typeof promoScoreNum === 'number' ? promoScoreNum : Number(promoScoreNum) || 0,
