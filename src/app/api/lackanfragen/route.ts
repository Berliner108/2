...
export async function GET(req: Request) {
  try {
    ...
    const applyFilters = (query: any) => {
      ...
      /**
       * Wichtig:
       * Wenn deine bestehenden Anfragen published=false haben,
       * würden sie mit published.eq.true ausgeblendet.
       * Deshalb filtern wir hier sauber über status.
       */
      if (!includeUnpublished) {
        next = next.neq('status', 'draft')
        next = next.neq('status', 'deleted')
      }
      ...
      /**
       * Achtung:
       * data->>menge ist ein Textwert.
       * Für den Start lassen wir den Filter trotzdem drin.
       * Langfristig wäre eine echte numerische Spalte besser.
       */
    ...
    /**
     * Count-Abfrage:
     * Zählt alle passenden Einträge für Pagination.
     */
    ...
    if (sortParam === 'promo') {
      /**
       * Promo-Sortierung:
       * Wir laden etwas mehr und sortieren danach stabil in JS.
       */
      const factor = 4
      const overTo = offset !== null
