        ...
        {slice.pageItems.map((order) => {
          ...
          return (
            <li key={`${order.kind}-${order.orderId}`} className={styles.card}>
              ...
              {/* rechte Spalte: nur Actions/Hinweise */}
<aside className={styles.side}>
  {isVendor && order.status === 'confirmed' && (
    <a href={`/api/invoices/${order.orderId}/download`} className={styles.secondaryBtn} target="_blank" rel="noopener">
  ...
  {isVendor && (order.status ?? 'in_progress') === 'in_progress' && (
    <div className={styles.actionStack}>
      <button type="button" className={styles.secondaryBtn} onClick={() => setShipOrder(order)}>
        Versandt melden
      </button>
  ...
  {isCustomer && order.status === 'reported' && (
    <div className={styles.actionStack}>
      <button type="button" className={styles.primaryBtn} onClick={() => doRelease(order)}>
        Empfang bestätigen & Zahlung freigeben
      </button>
      <button type="button" className={styles.btnGhost} onClick={() => openDispute(order)}>
        Problem melden
      </button>
...
{/* Adressen unten, volle Breite der Inhalts-Spalte */}
<section className={styles.addresses}>
  {isVendor && (order as any).shippingAddress && (
    <div className={styles.addrPanel} style={{ marginBottom: 8 }}>
