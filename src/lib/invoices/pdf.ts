...
export type InvoiceRenderData = {
  ...
  platform: {
    name: string
    vatId?: string
    // Adresse kann als einfacher Text in ENV kommen – wir zerlegen sie hier nicht hart:
    address?: Address | any
  }
  vendor: {
    displayName: string
  ...
  totals: {
    ...
    grossCents: number     // Brutto-Gesamt (Gebühr)
    netCents: number       // Netto (Gebühr)
    vatCents: number       // Steuerbetrag auf Gebühr
  }
  meta?: {
    orderId?: string
...
function addrToLines(a?: Address | any | null): string[] {
  if (!a) return []
  // Akzeptiert sowohl strukturierte Adressen als auch einfachen Text
  if (typeof a === 'string') {
...
export async function renderInvoicePdfBuffer(data: InvoiceRenderData): Promise<Buffer> {
  return new Promise<Buffer>((resolve, reject) => {
    ...
    // Zusätzliche Kennzahlen (Auftragswert / Auszahlung), wenn übergeben
