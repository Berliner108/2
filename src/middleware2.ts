...
function isLackList(path: string) {
  return path === '/lackanfragen' || path === '/lackanfragen/'
}
...
export async function middleware(req: NextRequest) {
  ...
  const guestOnlyPath = isGuestOnlyPath(pathname)

  /*
    Öffentliche Seiten direkt durchlassen,
    ABER Login/Registrieren nicht direkt durchlassen,
    weil eingeloggte User dort auf die Startseite umgeleitet werden sollen.
  */
  if (publicPath && !guestOnlyPath) {
  ...
  if (!url || !anon) {
    // Login und Registrierung sollen trotzdem erreichbar bleiben
    if (guestOnlyPath) {
      return NextResponse.next()
    }
  ...
  // Response vorab, damit Supabase Cookies aktualisieren kann
  const res = NextResponse.next()

  // Universal-Cookie-Adapter
  const cookieAdapter = {
  ...
  // Session wird nur einmal geprüft
  const {
    data: { session },
  } = await supabase.auth.getSession()

  // Eingeloggte User sollen nicht auf Login oder Registrierung bleiben
  if (session && guestOnlyPath) {
  ...
  // Eingeloggt → geschützte Seite erlauben
  if (session) {
    return res
  }

  // Nicht eingeloggt + Login/Registrierung → erlauben
  if (guestOnlyPath) {
    return res
  }

  // Nicht eingeloggt + geschützte Seite → Login mit Rücksprung
  const login = req.nextUrl.clone()
  login.pathname = '/login'
  login.searchParams.set('redirect', pathname + search)
